    ...
    function bindEvents(target) {
        ...
        $(target).unbind('.calendar').bind('mouseover.calendar', function (e) {
            ...
            if (t.hasClass('calendar-menu-next') || t.hasClass('calendar-nextyear')) {
                ...
                t.addClass('calendar-selected');
                var parts = t.attr('abbr').split(',');
                var dateTimeInput = common.DateFormater(opts.current, "yyyy-MM-dd hh:mm");
                var timeId = target.target.id;//获取当前时间控件id
                $("#" + timeId).datetimebox('setValue', dateTimeInput);
                $("#" + timeId).val(dateTimeInput);
                $("#" + timeId).datetimebox('hidePanel');
            }
        });
        function toTarget(t) {
