...
layui.define(['layer', 'setter', 'layRouter'], function (exports) {
    ...
    admin.flexible = function (expand) {
        ...
        if (isExapnd === expand) {
            if (expand) {
                ...
                $('.shrink-right').css({"transform":"rotate(0deg)","transition":"all ease 1s"})
            } else {
                $layout.addClass('admin-nav-mini');
                $('.shrink-right').css({"transform":"rotate(-180deg)","transition":"all ease 1s"})
    ...
    admin.req = function (url, data, success, method, option) {
        ...
        return admin.ajax($.extend({
            url: (conf.dpsimBaseServer || '') + url, data: data, type: method, dataType: 'json', success: success
        }, option));
    };
