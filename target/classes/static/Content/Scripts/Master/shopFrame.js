    ...
    $.fn.frame = function (options) {
        ...
        return this.each(function () {
            ...
            $(window).resize(function () {
                /////////////////////////////////////////////使用ALT键是页面会多少出空白  2019-01-10 陈剑辉 修改
                leftmenu.height($(window).height() - ((settings.hheight) + (settings.bheight)));//高度值变化
                ////当窗口发生改变栏目显示方位
                var width = 0;
                if (switchunfold.css("display") === "block") {
                    width = settings.switchwidth;
                } else {
                    width = settings.mwidth;
                }
                if (Sellerberleft.css("float") === "left") {
                    content.height($(window).height() - ((settings.hheight) + (settings.bheight))).css({ "width": $(window).width() - width, "margin-top": settings.hheight, "margin-left": width, "position": "relative" });
                }
                if (Sellerberleft.css("float") === "right") {
                    content.height($(window).height() - ((settings.hheight) + (settings.bheight))).css({ "width": $(window).width() - width, "margin-top": settings.hheight, "margin-right": width, "position": "relative" });
                }
                columnname();
            });
            //隐藏对象点击事件
            ...
            $(window).resize(function () {
                if (isHide == true) {
                    content.css({ "width": $(window).width(), "margin-left": "0px" });
                } else if (isMenuHide == false) {
                    content.css({ "width": $(window).width() - (settings.mwidth) });
                } else if (isMenuHide == true) {
                    content.css({ "margin-right": settings.mwidth, "width": $(window).width() - (settings.switchwidth) });
                } else {
            ...
            showbtn.bind("click", function () {
                ...
                if (leftmenu.width() === 0) {
                    hdatalist.find("table").animate({
                        width: $(window).outerWidth() - (settings.mwidth) - totalmargin
                    }, 500);
                }
                isHide = false;
            });
            ...
            setInterval(function () { $(datetime).html(currentTime()); }, 0);
            //判断高度切换样式
            ...
            $(document).on('click', columnlist, function () {
                $(this).on('click', '.submenu li a', function (e) {

                    // e.preventDefault();
                    e.stopPropagation();
                    if ($(this).attr("data-target") == "click") {
                        return;
                    }
                    if ($(this).attr('name')) {
                        var bStop = false;
                        var bStopIndex = 0;
                        var _href = "";
                        _href = $(this).attr('name');
                        var dataName = $(this).attr("data-name");
                        $("#dataNameUrlParam").val(dataName);
                        //_href = _href + "?dataName=" + dataName;
            ...
            $(columnlist).find('.submenu li.home').on('click', function (e) {
                $(columnlist).find('.submenu li.home').removeClass('active'); $(this).addClass('active');
            });
            //初始化事件
                ...
                if (data === false) {
                    ...
                    data.map(function (item) {
                        ...
                        if (item.child && item.child.length > 0) {
                            ...
                            //这个是.net判断页面跳转用的
                            if (item.target == "click") {
                                lihtml += "<a href='" + item.url + "' target='_blank' name='" + item.url + "' title='" + item.name + "' class='name menuUl_title' data-name='" + item.id + "' data-target='" + item.target + "'  ><i class='" + item.icon + "'></i>" + "<span id='" + item.id + "' class='menu-text'>" + item.name + "</span></a>";

                            } else {
                                //java 根据自己的业务进行判断的，目的是为了解决页面跳转不被拦截
                                if(item.url=="/dataKanban"){
                                    // console.log((item.url));
                                    lihtml += "<a href='" + item.url +"' target='_blank' name='" + item.url + "'  rel='noopener noreferrer' title='" + item.name + "' class='name menuUl_title' data-name='" + item.id + "'   data-target='click' ><i class='" + item.icon + "'></i>" + "<span id='" + item.id + "' class='menu-text'>" + item.name + "</span></a>";
                                }else {
                                    lihtml += "<a href='javascript:void(0)' name='" + item.url + "' title='" + item.name + "' class='name menuUl_title' data-name='" + item.id + "'   data-target='" + item.target + "'  ><i class='" + item.icon + "'></i>" + "<span id='" + item.id + "' class='menu-text'>" + item.name + "</span></a>";
                                }
                            }
                        }
                        ulHtml += lihtml;
            ...
            function bindEvent() {
                ...
                frame.find("#column_list").children("li").children("a").on("click", function (event) {
                    var ischek = $(this).attr("ischek");
                    frame.find(".menuUl li a").attr("ischek", 'false');
                ...
                frame.find(".menuUl li .submenu").hover(
                    function (event) {
                        if (Sellerberleft.hasClass("switch_menu")) {
                            var ischek = $(this).attr("ischek");
                            frame.find(".menuUl li a").attr("ischek", 'false');
                            var labeul = $(this).closest("li").find(">ul");
                            var parent = $(this).parent().parent();
                            ...
                            $(this).closest("li").addClass("open").find(labeul).show();
                        }
                ...
                frame.find(".menuUl li a").hover(function (event) {
                    if (Sellerberleft.hasClass("switch_menu")) {
                        var ischek = $(this).attr("ischek");
                        frame.find(".menuUl li a").attr("ischek", 'false');
                        var labeul = $(this).closest("li").find(">ul");
            ...
            function openIframe(href, titleName) {
                // loding.showLoding();
                var topWindow = $(window.parent.document);
                var show_nav = topWindow.find('#min_title_list');
                var iframe_box = topWindow.find('#iframe_box');

                if (!settings.multiTabs) {
                    //下面代码是为了防止打开多个页签
                ...
                if(href.indexOf("/dataKanban")==-1) {
                    ...
                    $("iframe[data-href='" + href + "']").load(function () {
                        // loding.hideLoding();
                    });
                }
            ...
            $(document).on("click", "#dropdown_menu .tabCloseCurrent", function () {
                ...
                if (aCloseIndex == 0) {
                    layer.alert('首页为锁定页，禁止关闭！', { icon: 4 });
                    return false;
                }
            ...
            $(document).on("click", "#dropdown_menu .tabCloseAll", function () {
                ...
                if ($tabCloseAll.not(".name").length) {
                    ...
                    $("#iframe_box").find('.show_iframe').not(".index_home").remove();
